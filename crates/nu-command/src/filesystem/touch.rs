...
impl Command for Touch {
                ...
                Some(reference) => {
                    ...
                    date = Some(
                        reference_path
                            ...
                            .into(),
                    ); // This should always be valid as it is available on all nushell's supported platforms (Linux, Windows, MacOS)
                ...
                if use_reference {
                    ...
                    if let Err(err) = filetime::set_file_atime(
                        ...
                        return Err(ShellError::ChangeAccessTimeNotPossible(
                            ...
                            call.positional_nth(index)
                                .expect("already checked positional")
                                .span, // if no date is given, then set the date to be today at 00:00
                        ));
                    };
                } else {
                    // Should not panic as we return an error above if we can't parse the date
    ...
    fn examples(&self) -> Vec<Example> {
        vec![
            ...
            Example {
                description:
                    "Changes the last modified time of files a, b and c to a date (local timezone)",
